...
export class RelayMonitor {
  ...
  async startMonitoring() {
    ...
    console.log('📡 Starting relay monitoring...')
    ...
    this.page.on('websocket', (ws) => {
      ...
      ws.on('framereceived', (payload) => {
        try {
          ...
          console.log('📥 WebSocket frame received:', JSON.stringify(data).substring(0, 100) + '...')
          ...
          // Skip non-JSON frames but only log for relay WebSocket
          if (ws.url().includes('10547')) {
            console.log('📥 Non-JSON relay frame received:', payload.toString().substring(0, 50) + '...')
          }
        }
      })
      ...
      ws.on('framesent', (payload) => {
        try {
          ...
          console.log('📤 WebSocket frame sent:', JSON.stringify(data).substring(0, 100) + '...')
          ...
          // Skip non-JSON frames but only log for relay WebSocket
          if (ws.url().includes('10547')) {
            console.log('📤 Non-JSON relay frame sent:', payload.toString().substring(0, 50) + '...')
          }
        }
      })
    ...
    this.page.on('console', (msg) => {
      const text = msg.text()
      if (text.includes('nostr') || text.includes('relay') || text.includes('EVENT') || text.includes('event')) {
        console.log('🖥️  Console log (nostr-related):', text)
        this.events.push({
          timestamp: Date.now(),
          type: 'received',
      ...
      profileEvents.forEach((event, index) => {
        ...
        if (data.type === 'websocket' && data.payload) {
          ...
          try {
            ...
            console.log(`  ${index + 1}. ${event.type} - Name: ${content.name || 'N/A'}, Display: ${content.display_name || content.displayName || 'N/A'}`)
          } catch (e) {
  ...
  async waitForProfileEvent(timeout = 10000): Promise<RelayEvent | null> {
    ...
    console.log('⏳ Waiting for profile event...')
    
    while (Date.now() - startTime < timeout) {
      const profileEvents = this.findProfileEvents()
      if (profileEvents.length > 0) {
        console.log('✅ Profile event found!')
        return profileEvents[profileEvents.length - 1] // Return latest
      }
      await new Promise(resolve => setTimeout(resolve, 500))
    }
    
    console.log('⏰ Timeout waiting for profile event')
    return null
  }
