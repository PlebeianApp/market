...
function testNoteRerenderingFix() {
    ...
    try {
        ...
        const hasReactionsIIFE = content.includes('const id = ((note as any)?.id || \'\') as string') && 
        ...
        const hasHashtagsIIFE = content.includes('const tagsArr = Array.isArray((note as any)?.tags)') &&
