...
const fetchMultipleProfiles = async (ndk: NDK, pubkeys: string[]): Promise<Map<string, NDKUserProfile | null>> => {
...
const followUser = async (ndk: NDK, pubkeyToFollow: string) => {
	...
	const existingContactList = existingEvents.size > 0 ? Array.from(existingEvents)[0] : null

	// Get existing p tags
	const existingPTags = existingContactList ? existingContactList.tags.filter((tag) => tag[0] === 'p') : []
	...
	event.tags = [...existingPTags, ['p', pubkeyToFollow]]
