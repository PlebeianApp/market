...
// Auction whitelist configuration (env-driven, backward-compatible)
//   APP_AUCTION_WHITELIST_MODE    — 'whitelist' to enforce, 'open' (default) to allow all
//   APP_AUCTION_WHITELIST_PUBKEYS — comma-separated hex pubkeys allowed to publish auctions
export const AUCTION_WHITELIST_MODE: 'whitelist' | 'open' =
	process.env.APP_AUCTION_WHITELIST_MODE === 'whitelist' ? 'whitelist' : 'open'
export const AUCTION_WHITELIST_PUBKEYS: string[] = (process.env.APP_AUCTION_WHITELIST_PUBKEYS || '')
	.split(',')
	.map((pk) => pk.trim())
	.filter((pk) => /^[0-9a-fA-F]{64}$/.test(pk))
