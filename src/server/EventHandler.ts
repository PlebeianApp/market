...
export class EventHandler {
  ...
  public async initialize(config: EventHandlerConfig): Promise<void> {
    ...
    this.ndkService = new NDKService(
      this.eventSigner.getAppPubkey(),
      this.adminManager,
      this.editorManager,
      this.bootstrapManager
    )
  ...
  private handleSpecialEvents(event: NostrEvent): void {
    const isSetupEvent = event.kind === 31990 && event.content.includes('"name":')
    const isAdminListEvent = event.kind === 30000 && 
      event.tags.some((tag) => tag[0] === 'd' && tag[1] === 'admins')
    const isEditorListEvent = event.kind === 30000 && 
      event.tags.some((tag) => tag[0] === 'd' && tag[1] === 'editors')
