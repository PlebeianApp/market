/**
 * NostrConnect URI emit + inbound-secret helpers (ADR-0002 signer-migration amendment I1, #807).
 *
...
export interface BuildNostrConnectUriArgs {
	...
	relay: string
	/** The connection secret (emitted as `secret=`, ADR-0002 signer-migration amendment I1 / #807). */
	secret: string
...
export function buildNostrConnectUri({ clientPubkey, relay, secret, metadata }: BuildNostrConnectUriArgs): string {
	...
	// output (ADR-0002 signer-migration amendment I1 / #807 fail-closed).
 ...
 * mismatch (fail closed, ADR-0002 signer-migration amendment I1 / #807).
 */
