...
interface Nip46LoginOptions {
	...
	/**
	 * Session passphrase for encrypted-at-rest persistence (ADR-0002 signer-migration amendment I5 /
...
export const authActions = {
	getAuthFromLocalStorageAndLogin: async () => {
		try {
			...
			// Signer / Bunker URL — legacy plaintext pair (ADR-0002 signer-migration amendment I5:
	...
	decryptAndLogin: async (password: string) => {
		try {
			...
			// NIP-49 lane via PasswordSigner (ADR-0002 signer-migration amendment): the ncryptsec is
	...
	loginWithPrivateKey: async (privateKey: string) => {
		...
		try {
			...
			// unchanged (ADR-0002 signer-migration amendment). No NEW plaintext is persisted here
	...
	loginWithExtension: async () => {
		...
		try {
			...
			// registry + capability seam, then attach an NDKSigner adapter so the
			// ~70 `signer.user()` consumers (and the wallet NWC paths) keep working
			// unchanged (ADR-0002 signer-migration amendment). No key is held locally.
			//
	...
	loginWithNip46: async (bunkerUrl: string, clientKeyHex?: string, options?: Nip46LoginOptions) => {
		...
		try {
			...
			// the signer capability seam (ADR-0002 signer-migration amendment §Decisions), then attach an
			...
			// Session persistence (ADR-0002 signer-migration amendment I5 / #996 H8): with a passphrase the
	...
	logout: () => {
		...
		// Lock on logout (ADR-0002 signer-migration amendment I5): the vaulted NIP-46 session is removed
	...
	/**
	 * Unlock prompt completion (ADR-0002 signer-migration amendment I5). With a legacy plaintext pair
	...
	unlockVaultedSession: async (passphrase: string, options?: WrapOptions & Partial<UnlockOptions>) => {
		...
		try {
			...
			if (preferredSessionSource() === 'legacy') {
				...
				// plaintext bearer capability rather than retain it (ADR-0002
				// signer-migration amendment I5 — never silently keep plaintext at rest).
				discardLegacySession()
			}
	...
	/**
	 * Unlock prompt refusal (ADR-0002 signer-migration amendment I5): the user declined the
