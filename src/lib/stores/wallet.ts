...
export const walletActions = {
	...
	addWallet: async (wallet: Omit<Wallet, 'id' | 'createdAt' | 'updatedAt'>, storeOnNostr: boolean): Promise<Wallet> => {
		...
		walletStore.setState((state) => {
			...
			if (storeOnNostr) {
				walletActions.saveWalletsToNostr(updatedWallets)
					.then(success => {
						if (!success) {
	...
	updateWallet: async (walletId: string, updates: Partial<Omit<Wallet, 'id' | 'createdAt'>>): Promise<void> => {
		walletStore.setState((state) => {
			...
			const shouldUpdateNostr = updatedWallets[walletIndex].storedOnNostr ||
				'storedOnNostr' in updates ||
				state.wallets.some(wallet => wallet.storedOnNostr);
