...
function ProfileComponent() {
	...
	useEffect(() => {
		if (pubkey) {
			console.log('🔄 Dashboard Profile: Triggering metadata load for user:', pubkey)
			...
			const loadUserMetadata = async () => {
				try {
					...
					if (result?.profile) {
						console.log('✅ Dashboard Profile: Successfully loaded metadata:', result.profile)
						...
						console.warn('⚠️ Dashboard Profile: No metadata found for user:', pubkey)
					}
				} catch (error) {
					console.error('❌ Dashboard Profile: Failed to load metadata:', error)
				}
			}
