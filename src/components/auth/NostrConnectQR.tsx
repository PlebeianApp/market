...
export function NostrConnectQR({ onError, onSuccess }: NostrConnectQRProps) {
	...
	const connectionUrl = useMemo(() => {
		...
		// #807 (ADR-0002 signer-migration amendment I1): the nostrconnect URI must carry the secret via the
			...
			sub.on('event', async (event: NDKEvent) => {
				...
				try {
					...
					if (request.method === 'connect') {
						...
						// #807 (ADR-0002 signer-migration amendment I1): the connect request must echo the secret via the
