			...
			<div className={`absolute inset-0 flex items-center ${backgroundImage ? 'dark' : ''}`}>
				<div className="w-full max-w-7xl mx-auto px-6">
					<div className={`flex flex-col ${getTextAlignmentClass()}`}>
						{/* Title */}
						{title && <h1 className="text-4xl md:text-5xl font-heading tracking-tight text-white mb-4">{title}</h1>}

						{/* Subtitle */}
						{subtitle && <p className="text-xl text-white/90 mb-8 max-w-2xl">{subtitle}</p>}
