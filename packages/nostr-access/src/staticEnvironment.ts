 ...
 */
import type { BrowseEnvironment, RawEvent, ReadResult, ThemeTokens } from './index'
...
export const createStaticEnvironment = (options: StaticEnvironmentOptions): BrowseEnvironment => {
	const wait = async () => {
	...
	return {
		...
		theme: { tokens: options.theme ?? defaultTheme, onChanged: () => () => {} },
		config: {
