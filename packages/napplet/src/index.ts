/**
 * `@plebeian/napplet` — the sandboxed napplet implementation of the contract.
 *
 * This is the piece that proves the architecture's central claim: **the components do not know which
 * implementation they are running under.** It maps the capability object a NIP-5D host injects
 * (`window.napplet`) onto `ModuleEnvironment`, and nothing above this file changes.
 *
 ...
 *   - `theme` → `theme.get` / `theme.changed`. The host exposes its token set; the contract's floor is
 *     only a last resort.
 ...
 *
 * It depends on `@plebeian/contract` and on nothing else shared: implementations do not share code with
 * each other, only the contract (spec: `packages/CONTRACT.md`).
 */
import type { ModuleEnvironment, QueryFilter, RawEvent, ReadOptions, ReadResult, ThemeTokens } from '@plebeian/contract'
import { CONTRACT_VERSION, TOKEN_FLOOR } from '@plebeian/contract'

/** The subset of a NIP-5D host's injected surface this implementation needs. Structural, not imported. */
export interface NappletRuntimeLike {
	outbox: {
		query(filters: readonly QueryFilter[], options?: { timeoutMs?: number }): Promise<readonly RawEvent[]>
		subscribe?(filters: readonly QueryFilter[], handler: (event: RawEvent) => void): { close(): void }
	}
	resource: {
...
export interface NappletEnvironmentOptions {
	...
	/** Resolved once at boot from `theme.get()`; the floor is only a last resort. */
	theme?: ThemeTokens
	now?: () => number
}

export const createNappletEnvironment = (options: NappletEnvironmentOptions): ModuleEnvironment => {
	...
	return {
		descriptor: {
			...
			grants: ['outbox:read', 'outbox:stream', 'resource', 'config', 'link'],
		},
		nostr: {
			read,
...
/**
 * Known limitation, recorded rather than hidden: `config.get` is synchronous in the contract and
 * asynchronous in the sandbox (`config.get` is a message round-trip). This implementation therefore
 * returns `undefined`, which means **a sandboxed surface cannot read the viewer's NSFW preference
 * synchronously** and must resolve it once at boot and hold it.
 *
 * That is a real interface gap, not a mapping detail: the host must be asked for its config before the
 * first render. Flagged as open question O1 in `docs/DECISIONS-packages-prototype.md`.
 */
