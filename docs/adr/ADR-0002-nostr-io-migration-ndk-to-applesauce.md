...
- **I2 — Remote signer ≠ authenticated user.** The remote-signer pubkey stays separate from the authenticated user pubkey, which is learned only via `get_public_key` / `getPublicKey()`. (Tests: `nostr-connect-signer.test.ts` — "identity resolution is via getPublicKey() only — never clientPubkey"; `nip46-signer-capability.test.ts` — identity-collapse cases.)
  ...
  > **Update (PR #1290, 2026-09-15):** I2 was first verified against the timeout-fallback path by PR #1290, which discovered that `master` unconditionally included the bunker pubkey as a user-identity candidate. The statement below that "PR #1199 is not an input dependency" stands for implementation strategy; it does not imply that the invariant was already correct on `master`. PR #1290's regression tests (`auth-nip46.test.ts` timeout-recovery scenarios, `auth.spec.ts` QR/bunker timeout with distinct keypairs) are the authoritative verification of I2 and must be carried forward by Wave A3b.
