...
const testEvents = [
  eventWithClientMostrTag,
  eventWithDifferentClientTag,
  eventWithoutClientTag,
  eventWithNoTags,
  eventWithNullTags
];
...
console.log('Filtered event IDs:', filteredEvents.map(e => e.id));

// Verify the correct event was filtered out
const shouldBeFiltered = testEvents.filter(e => hasClientMostrTag(e));
console.log('Events that should be filtered (client:Mostr):', shouldBeFiltered.map(e => e.id));