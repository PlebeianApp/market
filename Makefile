.PHONY: deploy-local deploy deploy-domain stop-local status-local status logs logs-local seed \
        dev build start format test check-deploy-env help \
        test-headed test-ui test-debug test-happy-path test-mcp-btc-price
...
test-happy-path: stop-local
	@echo "==> Starting happy path test environment..."
	@echo "Starting nak relay..."
	@nak serve --hostname 0.0.0.0 & sleep 2
	@echo "Seeding relay..."
	@bun run startup && bun run seed
	@echo "Starting currency server..."
	@NODE_ENV=development APP_RELAY_URL=ws://localhost:10547 bun run dev:currency-server & sleep 4
	@echo "Starting dev server..."
	@bun dev & sleep 6
	@echo ""
	@echo "==> Happy path environment ready!"
	@echo "    App:           http://localhost:3000"
	@echo "    Relay:         ws://localhost:10547"
	@echo "    Currency MCP:  running (dev mode, local relay only)"
	@echo ""
...
test-mcp-btc-price:
	@echo "==> Layer 1: Running unit tests (70 tests)..."
	@bun run test:unit
	@echo ""
	@echo "==> Layer 2: Running ContextVM.org tests (7 active, 4 skipped)..."
	@bunx playwright test --config=e2e-new/playwright-contextvm.config.ts
	@echo ""
	@echo "==> Layer 3: Running BTC price E2E tests..."
	@bunx playwright test --config=e2e-new/playwright.config.ts --grep "BTC price"
...
stop-local:
	...
	@echo "c03rad0r123" | sudo -S systemctl stop market nak currency-server 2>/dev/null || true
	@sleep 2
	@echo "==> Ports 3000 and 10547 cleared."

status-local:
	@echo "c03rad0r123" | sudo -S sh -c 'systemctl status market --no-pager 2>&1 || echo "market service not found"'
	@echo "---"
	@echo "c03rad0r123" | sudo -S sh -c 'systemctl status nak --no-pager 2>&1 || echo "nak service not found"'
	@echo "---"
	@echo "c03rad0r123" | sudo -S sh -c 'systemctl status currency-server --no-pager 2>&1 || echo "currency-server service not found"'

logs-local:
	journalctl -u market -u nak -u currency-server -f
...
status: check-deploy-env
	...
	ssh -i $(SSH_KEY_PATH) $(ANSIBLE_USER)@$(VPS_IP) \
		"systemctl status market --no-pager -l; echo '---'; systemctl status nak --no-pager -l; echo '---'; systemctl status currency-server --no-pager -l"
...
help:
	...
	@echo ""
	@echo "  Testing:"
	@echo "    make test-mcp-btc-price Run all BTC price tests (unit + E2E)"
	...
	@echo "    make test-happy-path Start full dev environment (nak + currency server + app)"
	@echo ""
	@echo "  Deployment:"
