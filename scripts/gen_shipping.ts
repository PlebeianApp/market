...
export function generateShippingData(): Omit<z.infer<typeof ShippingOptionSchema>, 'tags'> & { tags: NDKTag[] } {
  ...
  if (REGIONS[country as keyof typeof REGIONS]) {
    tags.push([
      "region",
      faker.helpers.arrayElement(REGIONS[country as keyof typeof REGIONS])
    ]);
  }
  ...
  tags.push([
    "price-weight",
    faker.number.float({ min: 0.5, max: 5, fractionDigits: 2 }).toString(),
    weightUnit
  ]);
  tags.push([
    "price-volume",
    faker.number.float({ min: 0.1, max: 2, fractionDigits: 2 }).toString(),
    dimensionUnit
  ]);
  tags.push([
    "price-distance",
