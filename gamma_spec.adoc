...
* Order communication and processing via link:https://github.com/nostr-protocol/nips/blob/master/17.md[NIP-17] encrypted messages
...
* Application preferences via link:https://github.com/nostr-protocol/nips/blob/master/89.md[NIP-89]
...
. Product discovery
. Cart addition
. Merchant preference verification
. Shipping calculation
. Payment processing
. Order confirmation
. Encrypted message follow-up

[#_payment_method_preferences]
==== Payment Method Preferences

Payment method related kind 0 fields in use and respected by most clients are the following two:

[source,jsonc]
{
    ...
    "lnurl": "lnurl1dp683skmn4wfk8qt...09emk2mrgtmq7n5",
    "lud16": "user@example.com",
    ...
}

This specification adds a new field:
...
In the event that the user overwrites their kind 0 event using another client that doesn't retain their payment preferences, they will find previously set preference overwritten. __TODO: how to deal with this - is this even a problem? perhaps can be resolved by application specific relays?__
...
. Application Preferences (link:https://github.com/nostr-protocol/nips/blob/master/89.md[NIP-89]):
* The recommended application MUST publish a kind link:https://github.com/nostr-protocol/nips/blob/master/89.md#handler-information[`31990`] event
* The merchant MUST publish a kind link:https://github.com/nostr-protocol/nips/blob/master/89.md#recommendation-event[`31989`] event recommending that application
. Payment Preferences:
* Set via `payment_preference` tag in the merchant's kind link:#_payment_method_preferences[`0`] event
...
* Checking kind link:https://github.com/nostr-protocol/nips/blob/master/89.md#handler-information[`31990`] events for recommended applications
* Checking kind link:#_payment_method_preferences[`0`] events for payment preferences
...
* `frequency`: _Optional_ subscription interval using ISO 8601 duration units (e.g. 'D' for daily, 'W' for weekly, 'Y' for yearly).
...
*** `extra-cost`: _Optional_ third element in the array, to add extra cost (in the product's currency) for the shipping method. In case of reference a collection the extra cost should be applied to all shipping options from the collection.
...
A specialized event type using link:https://github.com/nostr-protocol/nips/blob/master/51.md[NIP-51] like list format to organize related products into groups. Collections allow merchants or any user to create meaningful product groupings and share common attributes that products can also reference, establishing one-to-many relationships.
...
Products and collections can be saved as private drafts while being prepared for publication. This allows merchants to work on listings before making them publicly visible. Implementation MUST follow https://github.com/nostr-protocol/nips/blob/master/37.md[NIP-37] for draft management.
...
Order processing and status updates use link:https://github.com/nostr-protocol/nips/blob/master/17.md[NIP-17] encrypted direct messages, with three event kinds serving different purposes:
...
Product reviews follow https://github.com/nostr-protocol/nips/blob/b1432b705f553bde6c4eb5fcfde8525d2913b477/85.md[NIP-85] and https://habla.news/u/arkinox@arkinox.tech/DLAfzJJpQDS4vj3wSleum[QTS] guidelines with additional marketplace-specific rating criteria. Reviews provide structured feedback about products, merchants, and the overall purchase experience.
...
. eCash: Ideally the merchant has a kind `10019` event to know what mint they prefer.
...
. Lightning: Requires `lud16` or related lightning fields in kind `0`
...
* Merchant SHOULD have a link:https://github.com/nostr-protocol/nips/blob/master/89.md[NIP-89] kind link:https://github.com/nostr-protocol/nips/blob/master/89.md#recommendation-event[31989] event recommending their preferred service
...
* Merchant's link:https://github.com/nostr-protocol/nips/blob/master/89.md[NIP-89] application preferences SHOULD be respected
...
. Message threading should follow link:https://github.com/nostr-protocol/nips/blob/master/10.md[NIP-10] conventions when replies are needed.